...
import { EvaluationType, EvaluationStatus, EmployeeEvaluation } from "@/lib/types";
...
export function EditEvaluationDialog({
  ...
  // Populate form when evaluation changes
  useEffect(() => {
    if (evaluation) {
      setEvaluationType(evaluation.evaluationType);
      setFormData({
        name: evaluation.name,
        totalMarks: evaluation.totalMarks?.toString() || "",
        obtainedMarks: evaluation.obtainedMarks?.toString() || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/employees/${employeeId}/evaluations/${evaluation.id}`, {
        method: "PUT",
      ...
      if (response.ok) {
        ...
        alert(error.error || "Failed to update evaluation");
      }
    } catch (error) {
      ...
      <DialogContent className="sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Edit Evaluation for {employeeName}
          </DialogTitle>
          <DialogDescription>
            Update the test or interview evaluation details.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            >
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="test" id="edit-test" />
                <Label htmlFor="edit-test" className="font-normal cursor-pointer">
                  Test
                </Label>
              </div>
              <div className="flex items-center space-x-2">
                <RadioGroupItem value="interview" id="edit-interview" />
                <Label htmlFor="edit-interview" className="font-normal cursor-pointer">
                  Interview
                </Label>
              </div>
            </RadioGroup>
          ...
          {/* Evaluation Date */}
          <div className="space-y-2">
            <Label htmlFor="edit-evaluationDate" className="flex items-center gap-2">
              <Calendar className="h-4 w-4" />
              Evaluation Date
            </Label>
            <Input
              id="edit-evaluationDate"
              type="date"
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="edit-wasRetaken" className="font-normal cursor-pointer">
              This was a retake
            </Label>
          </div>
